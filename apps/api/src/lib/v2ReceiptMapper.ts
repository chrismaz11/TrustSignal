...
type FraudRiskSignal = {
    id: string;
    description: string;
    severity: RiskBand;
};

type ZkpAttestationPayload = {
    proofId: string;
    scheme: string;
    status: string;
    backend: string;
...
export function toV2VerifyResponse(input: {
    ...
    fraudRisk?: { score?: number; band?: RiskBand; signals?: FraudRiskSignal[] };
    zkpAttestation?: ZkpAttestationPayload;
    revoked?: boolean;
    riskScore?: number;
    ...
    const body: V2VerifyResponseBody = {
        receiptVersion: "2.0",
