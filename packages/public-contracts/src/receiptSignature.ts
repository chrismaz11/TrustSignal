...
import type { ReceiptSignature, UnsignedReceiptPayload } from './types.js';
...
export function canonicalizeUnsignedReceiptPayload(
...
export async function verifyReceiptSignature(
  ...
  try {
    ...
    const { payload: verifiedPayload, protectedHeader } = await compactVerify(
    ...
    const signatureMatchesMetadata =
      protectedHeader.alg === receiptSignature.alg &&
      protectedHeader.kid === receiptSignature.kid;
