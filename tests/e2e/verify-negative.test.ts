...
describeWithDatabase(`E2E /api/v1/verify fail-closed negative (${hasDatabase ? 'enabled' : databaseSkipReason})`, () => {
  ...
  it('fails closed when registry lookup is unavailable and does not report proof conformance', async () => {
    const synthetic = await curlJson(`${baseUrl}/api/v1/synthetic`, 'GET', undefined, 'e2e-read');
    expect(synthetic.status).toBe(200);

    const subjectName =
      typeof synthetic.body?.ocrData?.grantorName === 'string' && synthetic.body.ocrData.grantorName.trim().length > 0
        ? synthetic.body.ocrData.grantorName
        : 'ACME HOLDINGS LLC';
    ...
    const verify = await curlJson(`${baseUrl}/api/v1/verify`, 'POST', verifyInput, 'e2e-verify');
    expect(verify.status).toBe(200);
    expect(verify.body?.decision).toBe('FLAG');
    expect(verify.body?.decision).not.toBe('ALLOW');
    expect(Array.isArray(verify.body?.reasons)).toBe(true);
    expect((verify.body?.reasons ?? []).length).toBeGreaterThan(0);
    expect(verify.body?.zkpAttestation?.publicInputs?.conformance).toBe(false);
  });
});
